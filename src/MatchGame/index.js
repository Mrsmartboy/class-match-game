...
class MatchGame extends Component {
  state = {
    initialTabsId: tabsList[0].tabId,
  ...
  onTabAction = tabId => {
    this.setState({initialTabsId: tabId})
  }
  ...
  gameProcess = () => {
    const {initialTabsId, currentImageId} = this.state
    const filteredData = imagesList.filter(
      eachData => eachData.category === initialTabsId,
    )
    ...
    return (
      <div className="match-game-main-container">
        <div className="thumbnail-image-container">
          <img src={imageUrl} alt="match" className="thumbnail-img-1" />
        </div>
        <ul className="tabs-list-container">
          {tabsList.map(eachTab => (
            <Tabs
              ...
              isActive={initialTabsId === eachTab.tabId}
            />
          ))}
        </ul>
        <ul className="match-game-list-container">
          {filteredData.map(eachData => (
            <MatchGameItem
              eachData={eachData}
              key={eachData.id}
              onThumbnailAction={this.onThumbnailAction}
            />
          ))}
        </ul>
      </div>
    ...
    return (
      <div className="game-over-container">
        <div className="game-over-card">
          ...
          <h1 className="score-card">{score}</h1>
          <button
            type="button"
            className="reset-btn"
        ...
        <nav className="nav-main-container">
          ...
          <div className="nav-element-container">
            <ul className="timer-container">
              ...
              <li className="nav-items">
                <p className="timer-seconds">{timer} sec</p>
              </li>
            </ul>
          </div>
        </nav>
        {gameOver ? this.gameOver() : this.gameProcess()}
      </>
    )
  }
}
